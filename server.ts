...
// Initialize GoogleGenAI server-side with built-in authenticated credentials lazily
...
app.post("/api/gemini/cleanup-scraped-data", async (req, res) => {
  const { rawInput } = req.body;
  if (!rawInput) {
    res.status(400).json({ error: "Missing rawInput" });
    return;
  }
  ...
  try {
    ...
    res.json({ result: response.text ? response.text.trim() : "" });
  } catch (error: any) {
    console.error("Gemini Error (cleanupScrapedData):", error);
    res.status(500).json({ error: error?.message || "Gemini cleanup call failed" });
  }
});

app.post("/api/gemini/analyze-keywords", async (req, res) => {
  const { keywordData } = req.body;
  if (!keywordData) {
    res.status(400).json({ error: "Missing keywordData" });
    return;
  }
  ...
  try {
    ...
    res.json({ result: response.text || "Analysis failed to generate." });
  } catch (error: any) {
    console.error("Gemini Error (analyzeKeywords):", error);
    res.status(500).json({ error: error?.message || "Gemini analysis call failed" });
  }
});

app.post("/api/gemini/conduct-deep-dive", async (req, res) => {
  const { keywords, adCopy, landingPage } = req.body;
  ...
  try {
    ...
    res.json({ result: response.text || "Final report failed to generate." });
  } catch (error: any) {
    console.error("Gemini Error (conductDeepDive):", error);
    res.status(500).json({ error: error?.message || "Gemini deep dive call failed" });
  }
});
